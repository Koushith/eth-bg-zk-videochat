...
export const updateUser = async (req, res) => {
  try {
    ...
    // Add validation for achievements
    if (achievements) {
      const validAchievements = achievements.filter(
        (achievement) => achievement && achievement.name && achievement.name.trim() !== ''
      );
    ...
    if (error.name === 'MongooseError' && error.message.includes('buffering timed out')) {
      return res
        .status(500)
        .json({ success: false, message: 'Database operation timed out. Please try again.', error: error.message });
    }
...
export const getAchievementNames = async (req, res) => {
  try {
    console.log('getting achievement names route hit');
    const achievementNames = await User.distinct('achievements');
